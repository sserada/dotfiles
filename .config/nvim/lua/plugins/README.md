...
[設定ファイル](./ui/bufferline.lua)

画面上部にタブ風のバッファライン(開いているファイルの一覧)を表示するプラグイン。
...
- LSP診断（エラー・警告）をバッファラインに表示。
- neo-treeと連携してファイルツリー分のオフセットを自動調整。
- カラーアイコン付きで視覚的にわかりやすい。
- スラント型のセパレーターでモダンな見た目。

## toggleterm.lua

[設定ファイル](./integration/toggleterm.lua)
...
- デフォルトでフロート表示（画面中央にポップアップ）。
- 複数のターミナルを同時に管理可能。
- ターミナルモードから挿入モードへスムーズに移行。
- カーブしたボーダーで視覚的に美しい。

## indent-blankline.lua

[設定ファイル](./ui/indent-blankline.lua)

インデントレベルを縦線で可視化するプラグイン。コードの階層構造を一目で把握できます。

**主な機能:**

- 各インデントレベルに縦線を表示。
- 現在のスコープを強調表示。
- 特定のファイルタイプ（help、neo-tree、Troubleなど）では無効化。
- スコープの開始位置を表示（終了位置は非表示）。

## noice.nvim

[設定ファイル](./ui/noice.lua)

Neovimのコマンドライン、通知、メッセージUIを大幅に改善するプラグイン。モダンで美しいUIを提供。
...
- コマンドラインをポップアップ表示。
- 通知をモダンなUIで表示。
- LSPドキュメントのマークダウンレンダリング。
- 長いメッセージを自動的に分割表示。
- 下部検索、コマンドパレットのプリセット有効化。
...
[設定ファイル](./editing/flash.lua)

画面内の任意の位置へ素早く移動できるプラグイン。2-3キーストロークで目的の場所へジャンプ。

**キーマップ:**

| キー    | モード               | 説明                                   |
| ------- | -------------------- | -------------------------------------- |
| `s`     | Normal, Visual, Operator | Flash検索を開始。                     |
| `S`     | Normal, Visual, Operator | Treesitter構文ノードを選択してジャンプ。 |
| `r`     | Operator             | リモートFlash（離れた場所を操作対象に）。 |
| `R`     | Operator, Visual     | Treesitter検索。                      |
| `<C-s>` | Command              | Flash検索のトグル。                    |

**主な機能:**

- 画面内の任意の単語やパターンへ高速ジャンプ。
- Treesitterと統合して構文ノード単位での移動も可能。
- ビジュアルモードやオペレーターモードでも使用可能。
- f/t（文字検索）は無効化し、`s`キーでの検索に統一。
